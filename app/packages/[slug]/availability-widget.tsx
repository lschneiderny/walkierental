...
export default function PackageAvailability({ packageId }: { packageId: string }) {
  ...
  const check = async () => {
    ...
    try {
      const res = await fetch("/api/availability", {
        ...
        body: JSON.stringify({ packageId, startDate: start, endDate: end }),
      });
      const data = await res.json();
      const available: number = data.available ?? 0;
      if (available >= qty) {
        setResult(`Available: ${available} packages. You can add to cart.`);
        setCanAdd(true);
      } else {
        setResult(`Only ${available} packages available for these dates.`);
        setCanAdd(false);
      }
    } catch {
  ...
  const addToCart = async () => {
    await fetch("/api/cart", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ packageId, kind: "RENTAL", quantity: qty, startDate: start, endDate: end }),
    });
  };
